...
|------|------|------|------|
| 后端 API | 8443 (HTTPS) | — | — |
| PostgreSQL | 5432 | `changeme` | `changeme` |
| Redis | 6379 | — | （开发环境无密码） |
| MinIO API | 9000 | `changeme` | `changeme` |
| MinIO 控制台 | 9001 | `changeme` | `changeme` |
...
验证：
```bash
redis-cli -a changeme ping   # 返回 PONG
```
...
默认账号密码均为 `changeme`。在生产环境中应通过环境变量修改：
```bash
...
| 项目 | 开发环境状态 | 生产环境要求 |
|------|-------------|-------------|
| Redis 密码 | 已设置 (`changeme`) | 设置强密码 |
| Redis RDB 持久化 | 已关闭 (`save ""`) | 必须 `save ""` 关闭 |
| Redis AOF 持久化 | 已关闭 | 必须关闭 |
| MinIO 凭证 | `changeme/changeme` | 通过环境变量修改 |
...
**Q: 如何查看 MinIO 中的加密文件？**
A: 浏览器打开 `http://localhost:9001`，用 `changeme/changeme` 登录，进入 `vsec-videos` Bucket。文件以 `{uuid}/{videoId}.enc` 路径存储，内容为 SM4 加密密文，无法直接播放。
