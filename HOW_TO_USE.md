...
# 1. 复制并配置环境变量（必须配置邮箱，否则无法发送验证码）
cp .env.example .env

# 2. 编辑 .env，至少修改以下两项（缺一不可）：
#    MAIL_USERNAME=你的QQ邮箱@qq.com
#    MAIL_PASSWORD=你的QQ邮箱SMTP授权码
#    （授权码获取: QQ邮箱 → 设置 → 账户 → POP3/SMTP服务 → 开启）

# 3. 构建并启动所有服务
docker compose up -d --build
```

> **注意**：首次构建需要下载基础镜像和依赖，国内网络可能较慢（约 10~30 分钟）。已部署过的服务器后续只需 `docker compose up -d` 即可秒级重启，无需重复构建。
