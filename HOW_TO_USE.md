...
验证：
```bash
redis-cli -a vsec_redis ping   # 返回 PONG
```

**安全提醒**：生产环境必须为 Redis 设置密码，并关闭 RDB/AOF 持久化（见第九节）。

### 3.3 启动 MinIO（仅 MinIO 存储模式需要）
...
A: 检查 PostgreSQL 和 Redis 是否已启动（见第三节）。
